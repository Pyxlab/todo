...
export const Header: React.FC = () => {
    ...
    const sideNavOpen = useStore((state) => state.sideNavOpen);
    const toggleSideNav = useStore((state) => state.toggleSideNav);

    const asideOpen = useStore((state) => state.asideOpen);
    ...
    return (
        <header className="items-center grid grid-cols-[1fr_auto_1fr] gap-4 md:gap-0 md:flex">
            <button onClick={toggleSideNav} className="mr-6 block xl:hidden">
                <Equals size={24} />
            </button>
            <Dialog
                className="relative z-10 text-slate-600 dark:text-slate-400 xl:text-base sm:text-sm text-xs xl:hidden"
                open={sideNavOpen}
                onClose={toggleSideNav}
            >
                <Dialog.Overlay className="fixed inset-0 bg-black opacity-10" />
                <Dialog.Panel className="fixed left-0 top-0">
                    <SideNav />
                </Dialog.Panel>
            </Dialog>
            <SearchInput />
            ...
            <div className="flex flex-1">
                ...
                <Button onClick={toggleNewTask} className="sm:static fixed bottom-3 right-3 z-10 sm:z-0 min-w-max shadow-lg shadow-slate-400  dark:shadow-slate-900 sm:shadow-transparent">
                    Add new task
                </Button>
                ...
                >
                    <Dialog.Overlay className="fixed inset-0 bg-black opacity-10" />
                    <Dialog.Panel className="fixed right-0 top-0">
                        <Aside />
                    </Dialog.Panel>
                </Dialog>
            </div>
